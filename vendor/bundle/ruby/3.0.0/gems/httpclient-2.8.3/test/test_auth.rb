...
class TestAuth < Test::Unit::TestCase
  ...
  def setup_server
    ...
    ntlm_handler = Rack::Handler::WEBrick.new(@server,
      Rack::Builder.app do
        ...
        use Rack::Ntlm, {:uri_pattern => /.*/, :auth => {:username => "elm3ml", :password => "elm3ml"}}
  ...
  def test_ntlm_auth
    c = HTTPClient.new
    c.set_auth("http://localhost:#{serverport}/ntlm_auth", 'elm3ml', 'elm3ml')
    assert_equal('ntlm_auth OK', c.get_content("http://localhost:#{serverport}/ntlm_auth"))
  end

  def test_basic_auth
    c = HTTPClient.new
    c.set_auth("http://localhost:#{serverport}/", 'elm3ml', 'elm3ml')
    assert_equal('basic_auth OK', c.get_content("http://localhost:#{serverport}/basic_auth"))
  end

  def test_basic_auth_compat
    c = HTTPClient.new
    c.set_basic_auth("http://localhost:#{serverport}/", 'elm3ml', 'elm3ml')
    assert_equal('basic_auth OK', c.get_content("http://localhost:#{serverport}/basic_auth"))
  end
  ...
  def test_perfer_digest
    c = HTTPClient.new
    c.set_auth('http://example.com/', 'elm3ml', 'elm3ml')
  ...
  def test_proxy_auth
    c = HTTPClient.new
    c.set_proxy_auth('elm3ml', 'elm3ml')
  ...
  def test_digest_proxy_auth_loop
    ...
    c.test_loopback_http_response << "HTTP/1.0 200 OK\nContent-Length: 2\n\nOK"
    md5 = Digest::MD5.new
    ha1 = md5.hexdigest("elm3ml:foo:elm3ml")
  ...
  def test_negotiate_and_basic
    ...
    c.test_loopback_http_response << %Q(HTTP/1.0 200 OK\r\nConnection: Keep-Alive\r\nContent-Length: 1\r\n\r\nb)
    c.debug_dev = str = ''
    c.set_auth('http://www.example.org/', 'elm3ml', 'elm3ml')
    # Do NTLM negotiation
